...
Server.prototype.wireAppEvents = function(app)
{
  var self = this;
  app.on('device:update', function(device) {
    self.broadcast(JSON.stringify(
      {'type' : 'device', 'data' : device.toJSON()}
    ));
  });

  app.on('sensor:update', function(sensor) {
    self.broadcast(JSON.stringify(
      {'type' : 'sensor', 'data' : sensor.toJSON()}
    ));
  });
}
...
Server.prototype.socketConnection = function(client)
{
  ...
  client.on('message', function(message) {
    ...
    if (msgMeta.valid) {
      ...
      client.send(JSON.stringify({'type' : 'error', 'message' : msgMeta.error }));
    }
  });
}
